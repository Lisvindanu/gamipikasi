...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
    <div>
        ...
        <p style="color: var(--text-secondary);">Manage tasks for your department members</p>
    </div>
    <a href="{{ route('head.dashboard') }}" class="btn btn-secondary">
...
<!-- Cari & Filter Bar -->
<div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: var(--shadow-md); margin-bottom: 2rem;">
    <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 1rem;">
        ...
        <div style="position: relative;">
            ...
            <input type="text" id="taskCari" placeholder="Cari tasks..." style="width: 100%; padding: 0.75rem 0.75rem 0.75rem 2.5rem; border: 2px solid var(--border-color); border-radius: 12px; font-size: 0.9375rem;" oninput="filterTasks()">
        </div>

        <!-- Status Filter -->
        <select id="statusFilter" style="padding: 0.75rem; border: 2px solid var(--border-color); border-radius: 12px; font-size: 0.9375rem; cursor: pointer;" onchange="filterTasks()">
        ...
        <!-- Prioritas Filter -->
        <select id="priorityFilter" style="padding: 0.75rem; border: 2px solid var(--border-color); border-radius: 12px; font-size: 0.9375rem; cursor: pointer;" onchange="filterTasks()">
        ...
        <!-- Reset Button -->
        <button onclick="resetFilters()" class="btn btn-secondary" style="display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
...
<!-- My Tasks (Assigned by Lead) -->
@if($myTasks->count() > 0)
<div style="background: white; border-radius: 20px; padding: 2rem; box-shadow: var(--shadow-md); margin-bottom: 2rem;">
    <h2 style="font-size: 1.25rem; font-weight: 700; color: var(--text-primary); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                ...
                <div class="card-meta">
                    ...
                    @if($task->point_reward)
                        <div class="card-meta-item">
                            ...
                            <span>{{ $task->point_reward }}pt</span>
                        </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    // Upload Evidence (for tasks assigned to head)
    async function uploadEvidence(event, taskId) {
        ...
        try {
            const response = await fetch(`/head/tasks/${taskId}/attachments`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json',
            ...
            if (data.success) {
                // Store taskId in sessionStorage to reopen modal after reload
    ...
    async function addTaskComment(event, taskId) {
        ...
        try {
            const response = await fetch(`/head/tasks/${taskId}/comments`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
